...
function dloaAnalysisMessage(hire, analysis, score) {
  const scoreEmoji = score == null ? '' : score >= 7 ? ' ⬆️' : score >= 5 ? ' ➡️' : ' ⬇️'
  const scoreLine = score != null ? `*Score: ${score}/10*${scoreEmoji}` : null
  ...
  if (scoreLine) {
    blocks.push({
      type: 'context',
      elements: [{ type: 'mrkdwn', text: scoreLine }]
    })
  }
